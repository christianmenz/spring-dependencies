...
public class SpringDepdendencyMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        try {
            ...
            URLClassLoader classLoader = new URLClassLoader(urlList.toArray(new URL[0]), getClass().getClassLoader());
           
            SpringDependencyApplicationContext context = new SpringDependencyApplicationContext(printUrl);
            context.setClassLoader(classLoader);            
            context.setConfigLocations(configLocations);
            context.refresh(); // trigger config file loading
        } catch (Exception e) {
            e.printStackTrace();
            throw new MojoExecutionException("Failed to process Spring configuration");
        }
    }
